...
Handle g_hCookie_Disableredirect = INVALID_HANDLE;
...
{
	gServerCommands = CreateConVar("server_redirect_commands", "sm_list;sm_hop", "Commands that invoke servers redirect menu.\n(Note: Map change required for changes to take effect! There's also a 128 character limit per command.)");
	...
	gAdvertisementMinPlayers = CreateConVar("server_redirect_advertisement_min_players", "5", "Minimum players required to advertise server in chat.\n(Note: Unused if you don't use socket extension)", .hasMin = true);
	...
	gAnnounceLeave = CreateConVar("server_redirect_show_advertisement_leave", "1", "If set, will show message to alert players a player has connected to another server.", .hasMin = true, .hasMax = true, .max = 1.0);
	...
	RegConsoleCmd("sm_nohop", Command_Nohop, "nohop");
	...
	g_hCookie_Disableredirect = RegClientCookie("cooike_redirect", "Disable Redirect", CookieAccess_Private);
	InitNet(gd);
	SetupSDKCalls(gd);
	SetupDhooks(gd);
...
public Action Command_Nohop(int client, int iArgs)
{
	
	if(!g_bDisabled[client]) 
	{
	g_bDisabled[client] = true;
	SetClientCookie(client, g_hCookie_Disableredirect, "1");
	
	}
	ReplyToCommand(client,"分流插件已经关闭.");
...
public void OnClientCookiesCached(int client)
{
	...
	GetClientCookie(client, g_hCookie_Disableredirect, sBuffer_cookie, sizeof(sBuffer_cookie));
	//PrintToServer("%s",sBuffer_cookie);
	g_bDisabled[client] = view_as<bool>(StringToInt(sBuffer_cookie));
	int clients = GetClientCount(false);
	int limit = GetMaxHumanPlayers() - 2;
	//PrintToServer("%b",g_bDisabled[client])
	
		if (clients >= limit && !g_bDisabled[client])
		{
...
bool FilterServerToAdvert(ServerEntry se, int idx = 0)
{
	...
	if(gSocketAvaliable && (se.maxplayers == 0 
				|| (se.curr_players > 60 && se.curr_players_info > 60)
