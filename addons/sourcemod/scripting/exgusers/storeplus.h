...
Action Command_QD(int client,int args)
{
	...
	else
	{
		...
		{
			PrintToChat(client, " \x05[签到系统] \x01距离上一轮签到已经过去七天了,进入新一轮签到轮次");
	...
	{     
		if(!isvip){
			...
			if(isExgTagPlayer){
				int add = 20;
				PrintToChat(client, " \x05[签到系统] \x01携带 EXG 组标,签到加赠%d积分", add);
			}
		}
		if(firstDay){
			if(isvip){
				PrintToChat(client, " \x05[签到系统] \x01完成签到获得%d积分(已享VIP双倍加成)", credits_bonus);
			}else{
				PrintToChat(client, " \x05[签到系统] \x01完成签到获得%d积分(若您为VIP则为双倍)", credits_bonus);
			}
		}else{
			PrintToChat(client, " \x05[签到系统] \x01完成本轮连续%d天签到,共获得%d积分!",g_Users_sp[client].qd,credits_bonus);
		}
